...
import { ArrowLeft, User, Mail, Phone, Check, X, Clock, FileX, Pencil, CalendarCheck } from 'lucide-react'
...
const STATUS_CONFIG = {
  present: { icon: Check,  bg: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400',  dot: 'bg-green-500' },
  absent:  { icon: X,      bg: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400',          dot: 'bg-red-500' },
  late:    { icon: Clock,  bg: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400', dot: 'bg-yellow-500' },
  excused: { icon: FileX,  bg: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400',      dot: 'bg-blue-500' },
} as const
...
export function StudentProfilePage() {
  ...
  const stats = records.reduce(
    (acc, r) => { acc[r.status as Status] = (acc[r.status as Status] ?? 0) + 1; return acc },
    {} as Record<Status, number>
  )
  const totalInPage = records.length
  const presentCount = stats.present ?? 0
  const totalAll = meta?.total ?? 0
  const presentRate = totalAll > 0
    ? Math.round(((historyData?.data ?? []).filter((r: AttendanceRecord) => r.status === 'present').length / totalInPage) * 100)
    : 0
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 mb-6">
            <div className="flex flex-col sm:flex-row items-start gap-5">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-2xl font-extrabold text-gray-900 dark:text-gray-100">{s.full_name}</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/60 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800">
                  <th className="text-left px-3 md:px-6 py-3 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('dateOfBirth').replace('Date of Birth', 'Date')}</th>
                  <th className="text-left px-3 md:px-6 py-3 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('markAttendance').replace('Mark Attendance', 'Status')}</th>
                  <th className="text-left px-3 md:px-6 py-3 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden sm:table-cell">Notes</th>
                </tr>
              </thead>
              <tbody>
                {historyLoading
                      ...
                      return (
                        <tr
                          ...
                          className={`border-b border-gray-50 dark:border-gray-800 last:border-0 ${
                            i % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50/30 dark:bg-gray-800/30'
                          }`}
                        >
